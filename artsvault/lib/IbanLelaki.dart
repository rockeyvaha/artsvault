...
class IbanLelaki extends StatelessWidget {
  const IbanLelaki({super.key});
...
class IbanLelakiPage extends StatefulWidget {
  const IbanLelakiPage({super.key});

  @override
  _IbanLelakiPageState createState() => _IbanLelakiPageState();
}

class _IbanLelakiPageState extends State<IbanLelakiPage> {
          ...
          Stack(
            children: [
              IconButton(
                ...
                onPressed: () {
                  // Navigate to the CartPage when the cart icon is clicked
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) =>
                            const CartPage()), // Redirect to CartPage
                  );
                },
              ),
              if (cartCount > 0)
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Center(
              child: Image.asset(
                'assets/iban2.jpg', // Replace with the actual Iban costume image
                height: 200,
              ),
            ),
            ...
            const Center(
              child: Text(
                'The traditional male attire includes the baju burung (bird shirt) and sirat (loincloth). The baju burung is often worn during festivals and ceremonies, symbolizing bravery and strength.',
                style: TextStyle(
                  fontSize: 16,
                  color: Colors.grey,
                ),
                ...
                onPressed: () {
                  showModalBottomSheet(
                    ...
                    builder: (context) => AddToCartPopup(
                      onAddToCart: _addToCart, // Pass the callback here
                    ),
                  );
                },
                child: const Text(
...
class AddToCartPopup extends StatefulWidget {
  final void Function(int quantity) onAddToCart; // Add this line to define the callback function

  const AddToCartPopup({super.key, required this.onAddToCart}); // Make sure it is passed in the constructor
